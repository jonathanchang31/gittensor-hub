...
import { Text, Box, Label, Link as PrimerLink } from '@primer/react';
import {
  RepoIcon,
  StarIcon,
  StarFillIcon,
...
export default function PullsTable({ repo }: { repo?: string } = {}) {
  ...
  const currentRepoNames = useMemo(() => {
    ...
    for (const r of sn74Repos) names.set(r.fullName.toLowerCase(), r.fullName);
    return names;
  }, [sn74Repos]);
  ...
  return (
    <Box sx={{ width: '100%', ...(repo ? {} : { maxWidth: PULLS_CONTENT_MAX_WIDTH, mx: 'auto' }) }}>
      <Box
        sx={{
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <Box as="table" sx={{ width: '100%', minWidth: repo ? 900 : 1200, tableLayout: 'fixed', borderCollapse: 'collapse', fontSize: 1 }}>
          ...
          <Box as="tbody">
            {isLoading && rows.length === 0 && (
              <Box as="tr">
                <Box as="td" colSpan={repo ? 9 : 11} sx={{ p: 0 }}>
                  <TableRowsSkeleton
                    rows={12}
                    cols={
                      repo
            ...
            {!isLoading && rows.length === 0 && (
              <Box as="tr">
                <Box as="td" colSpan={repo ? 9 : 11} sx={{ p: 4, textAlign: 'center', color: 'var(--fg-muted)' }}>
                  {isError
